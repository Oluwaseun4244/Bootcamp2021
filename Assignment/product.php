    ...
    <?php if (isset($_POST["Upload"])) {
      ...
      foreach ($productCsv as $key => $r) {
        if ($key > 0) {
          ...
          // $subTotal = "";
          if ($amount >= 0) {
            $subTotal = $defSubTotal;
            $bc = 0.07 * $subTotal;
            $cg = 0.05 * $subTotal;
            $total = $subTotal + $bc + $cg;
          } else {
            $subTotal = 0;
          }
    ...
    ?>
          <div class="row mt-3 col-xs-12">
            ...
            <div class="col-sm-6 center">
              <h5><?php echo $r[2]; ?></h5>
              <h6>ITEM: <?php echo  $r[1]; ?></h6>
              <p>Price: <?php echo "$" . $r[3]; ?></p>
              <p><i class="fa fa-cart-arrow-down fa-lg"></i> Qty: <?php echo $r[4]; ?></p>
            </div>
            <div class="col-sm-2" id="amount">
              <p class="text-end"><?php echo "$" . $amount; ?></p>
            </div>
          </div>
          <hr>
      <?php }
      ...
      <div id="total-div">
        <span>
          ...
          <h6>British Colombia PST(7%): <?php echo "$" . $bc ?></h6>
          <h6>CANADA GST(5%): <?php echo "$" . $cg ?></h6>
          <h6>TOTAL: <?php echo "$" . $total ?> <i class="fa fa-credit-card-alt" aria-hidden="true"></i></h6>
        </span>
      </div>
    <?php
    } else { ?>
